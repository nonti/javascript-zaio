...
//destructuring set to an empty object
products.forEach(
...
class ShoppingCart {
    ...
    addItem(id, products) {
        ...
        this.items.forEach((dessert) => {
            totalCountPerProduct[dessert.id] = (totalCountPerProduct[dessert.d] || 0) + 1;
        })

        const currentProductCount = totalCountPerProduct[dessert.id];
        ...
        currentProductCount > 1
            ...
            <div id="dessert${id}" class="product">
                <p>
                    <span class="product-count" id="dessert${id}">${name}</span>
                </p>
                <p>${price}</p>
            </div>
            `;
...
[...addToCartBtn].forEach(
    (btn) => {
        btn.addEventListener('click', (event) => {
            event.addItem(Number(event.target.id), products);
        })
    }
);
